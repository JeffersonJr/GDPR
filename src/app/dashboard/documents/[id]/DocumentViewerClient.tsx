    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link href="/dashboard" className="p-2 hover:bg-slate-800 rounded-lg transition-colors text-slate-400 hover:text-white">
            <ArrowLeft size={20} />
          </Link>
          <div>
            ...
            <p className="text-sm text-slate-400">
              {doc.name} • Status: <span className="text-slate-300 capitalize">{doc.status}</span>
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {doc.compliant_url && (
            <a
              ...
              download
              target="_blank"
              className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-sm font-semibold flex items-center gap-2 transition-colors border border-slate-700"
            >
              <Download size={16} />
              Exportar
            </a>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="glass-card p-5 space-y-4">
              ...
              <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                {analysis.issues.map((issue, idx) => (
                  <div key={idx} className="p-3 bg-slate-800/50 rounded-xl border border-slate-700/50 text-sm space-y-2">
                ...
                {analysis.issues.length === 0 && (
                  <p className="text-sm text-slate-400">Nenhum problema encontrado. Excelente!</p>
                )}
              </div>
            </div>
          </div>
        ...
        <div className={clsx("glass-card flex flex-col", analysis ? "lg:col-span-9" : "lg:col-span-12")}>
          <div className="flex items-center justify-between px-2 py-2 border-b border-slate-800/60 bg-slate-900/50">
            <div className="flex p-1 bg-slate-800/50 rounded-lg">
              <button
                onClick={() => setActiveTab('original')}
                className={clsx(
                  "px-4 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-2",
                  activeTab === 'original' ? "bg-slate-700 text-white shadow-sm" : "text-slate-400 hover:text-slate-200"
                )}
              >
              ...
              <button
                ...
                className={clsx(
                  ...
                  activeTab === 'improved' ? "bg-brand-600 text-white shadow-sm" : "text-slate-400 hover:text-slate-200"
                )}
              >
          ...
          <div className="flex-1 p-6 min-h-[60vh] bg-slate-950 rounded-b-2xl relative">
            {activeTab === 'original' ? (
              ...
              isEditing ? (
                <textarea
                  ...
                  className="w-full h-full min-h-[60vh] bg-transparent border-none focus:ring-0 text-slate-300 font-mono text-sm leading-relaxed resize-none"
                  spellCheck="false"
                />
              ) : (
                <div className="prose prose-invert prose-brand max-w-none text-slate-300 text-sm">
