...
import { Eye, EyeOff, Loader2, ShieldCheck, Mail, Lock, Code } from 'lucide-react'
import { toast } from 'sonner'
import { devAutoLoginAction } from '@/lib/actions'
...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (error) {
      toast.error('Falha na autenticação', { description: 'E-mail ou senha incorretos.' })
      setLoading(false)
      return
    }
  ...
  async function handleDevLogin() {
    setLoading(true)
    const res = await devAutoLoginAction()
    if (res?.error) {
      toast.error('Erro no modo dev', { description: res.error })
      setLoading(false)
    }
  }
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        >
          {loading
            ...
            : <><ShieldCheck size={18} /> Entrar com segurança</>
          }
        </button>
      </form>
